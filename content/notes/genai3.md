...
$$
p_x(x') = p_z(z) \left| det[J_f] \right|
$$

Intuitively, we can also write 

$$
p_z(z) = p_x(x') \frac{1}{\left| det[J_f] \right|} = p_x(x') \left| det[J_{f^{-1}}] \right|
$$

>[!summary] Explaining the Jacobian
>
> $J_f$ is the Jacobian of the model from **x to z** (the inverse direction).
>
> We can write in terms of probability density function (see [[changeofvar|Change-of-Variable Formula]] theorem)
>
> Since $\mathbf{x} = f(\mathbf{z})$, we have:
>
>$$
>J_f = \frac{\partial \mathbf{z}}{\partial \mathbf{x}} = \frac{\partial f^{-1}(\mathbf{x})}{\partial \mathbf{x}}
>$$
>
>It describes how the transformation $f$ maps from the **data space (x) to the latent space (z)**.
>
> Conversely, $J_{f^{-1}} = \frac{\partial \mathbf{x}}{\partial \mathbf{z}} = \frac{\partial f}{\partial \mathbf{z}}$ maps from z to x.
>
> $$
...
>The function $f$ in normalizing flows is perfectly invertible. In normalizing flows, we care about density estimation, not reconstruction. The loss is based on the [[likelihood function|log likelihood]] of data $\mathbf{x}$ under the model (more below).
>
...
* In **training**, data flows from $x \rightarrow z$ where $z = f^{-1}(x)$. We maximize $\log p_x(x')$ (or minimize the negative log likelihood): 

$$
\log p_z(z) = \log p_x(x) + \log \left| det[J_{f^{-1}}] \right|
$$

$$
\log p_x(x) = \log p_z(z) + \log \left| det[J_f] \right|
$$
...
$$
\log p_z(z) = \log p_x(f(z)) + \log \left| \det[J_{f^{-1}}] \right|
$$
...
$$
\mathcal{L} = -\log p_x(x) = -\log p_z(z) + \log \left| \det \left( \frac{\partial f}{\partial \mathbf{z}} \right) \right|
$$
