---
title: AI for Robotics
draft: false
tags:
date: 2026-05-12
---
 
Mandatory course I took in Q4 of my first year while at [[twente|Twente]]. It covered mostly information from the [[IPCV|Image Processing and Computer Vision]] course and overall computer-vision related tasks that can be done with AI (e.g. YOLO for detection and tracking, MiDAS for depth estimation).

>[!summary] Topics covered
>
>* [[ai for robotics 1|Detectors and Tracking]]
>* [[RPCN 14|Depth Accuracy for Stereo Cameras]]
>* [[camera calibration and 3d recontruction|Stereo Reconstruction]], [[RPCN 15|Bundle Adjustment]]
>* [[RPCN 6|SLAM, but mostly covered the camera oriented side]]
>* [[MiDaS]]
>* [[ai for robotics 2|Machine Learning Paradigms]]

